<template>
    <div class="mt-5">
        <h5 class="mb-5 text-2xl font-medium text-gray-700">{{ $t('product.recommended') }}</h5>
        <vue-horizontal class="container flex items-center mx-auto ">
            <section class="flex flex-row space-x-5" v-if="products">
                <div class=" md:w-64 w-44" v-for="product in products" :key="product.id">
                    <a class="" :href="$router.resolve({ name: 'DefaultProduct', params: { id: product.id, slug: sanitizeTitle(product.name ? product.name : product.vendor.name) }}).href">
                        <img :src="product.photo" class="h-40 w-44" alt="">
